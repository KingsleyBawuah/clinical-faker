...
- HL7 v2 common segment builders (`src/hl7/segments`): `buildMSHSegment`/`buildPIDSegment`/`buildPV1Segment`, plus supporting `toDTM()` timestamp formatting, deterministic `MSH-10` generation, `xpn`/`xad`/`cx`/`xcn` composite-datatype helpers, and gender/patient-class coded-value mappings. Not yet wired into `createPatient()`/`.toHL7()`.
