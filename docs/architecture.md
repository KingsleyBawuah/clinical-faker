...
These are pure functions with no fallback branch needed — every IR union is closed and every arm maps to exactly one HL7 code, so a `never`-exhaustiveness check at each mapping function catches an unmapped case at compile time if the IR's union ever grows (`assertExhaustive`, `src/hl7/mappings/`).

**Composite datatype helpers** (`src/hl7/composites.ts`) wrap a plain IR value into the `HL7Value` shape a specific HL7 v2 composite datatype expects, with component order confirmed against HL7 v2.5.1 rather than assumed: `xpn()` (Extended Person Name: family^given), `xad()` (Extended Address: street^other-designation^city^state^zip^country), `cx()` (Extended Composite ID: id^check-digit^check-digit-scheme^assigning-authority), `xcn()` (Extended Composite ID + Name for Persons: id^family^given), `msg()` (`MSH-9`'s own Message Type composite: code^trigger-event^structure — added after external review caught that `buildMSHSegment`'s `messageType` field needed the same wrap as every other composite but had no dedicated helper). Where this library's IR has no equivalent for a component (`XAD`'s "other designation", `CX`'s check-digit fields), that component is left empty rather than dropped, so it doesn't shift every component after it out of position. Future segments needing a coded-element composite (`CE`, for `DG1-3`/`AL1-3`/`OBX-3` once those land) should add a same-shaped helper here rather than hand-building the nested array at the call site — confirming its component order the same way before relying on it.
